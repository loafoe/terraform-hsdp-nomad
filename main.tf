...
resource "hsdp_container_host_exec" "init" {
  ...
  file {
     source = templatefile("${path.module}/templates/agent.hcl", {})
  ...
  commands = [
    ...
    "docker run -d --name nomad -v nomad:/config -p8282:4646 -e DOCKER_HOST=tcp://${hsdp_container_host.nomad.private_ip}:2375 ${var.nomad_image} /app/nomad agent -dev -bind=0.0.0.0 -acl-enabled -plugin-dir=/plugins -config=/config/agent.hcl",
    "sleep 5",
